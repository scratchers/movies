    ...
    public function testAdminCanCreateMovie()
    {
        $this->browse(function ($browser) {
            $browser->visit('/login')
                    ...
                    ->assertPathIs('/movies')
                    ->assertDontSee('film')
                    ->assertVisible('#link-create-movie')
                    ->visit(
                        $browser->attribute('#link-create-movie', 'href')
                    )
                    ...
                    ->assertSee('film')
                    ->assertVisible('#link-edit-movie-1')
                    ->visit('/movies')
                    ->assertSee('film')
            ;
        });
    ...
    public function testAdminCanGroupMovie()
    {
        $this->browse(function ($browser) {
            $browser->visit('/movies')
                    ...
                    ->assertSee('movie.mkv')
                    ->assertVisible('#link-edit-movie-2')
                    ->visit(
                        $browser->attribute('#link-edit-movie-2', 'href')
                    )
                    ->assertPathIs('/movies/2/edit')
    ...
    public function testAdminCanDestroyMovie()
    {
        $this->browse(function ($browser) {
            $browser->visit('/login')
                    ...
                    ->assertVisible('#link-edit-movie-1')
                    ->visit(
                        $browser->attribute('#link-edit-movie-1', 'href')
                    )
